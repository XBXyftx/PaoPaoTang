...
public class StarList extends ArrayList<Star>{
	...
	public static void main(String[] args){
		...
		for(int i = 0;i<9;i++){
			starList.add(new Star(new Position(i,0),StarType.BLUE));
		}
//		System.out.println(starList.lookup(3, 0));
//		System.out.println(starList.lookup(1, 1));
		System.out.println(starList.lookup(new Position(3,0)));
		System.out.println(starList.lookup(new Position(1,1)));
	}
	...
	public Star lookup(Position po){
		Star star = new Star();
		for(int i = 0;i<this.size();i++){
			star = this.get(i);
			if(star.getPosition().getRow()==po.getRow()&&star.getPosition().getColumn()==po.getColumn()){
				return star;
			}
		}
		return null;
	...
	public boolean existed(Star star){
		if(star==null){
			...
			if(lookup(star.getPosition())!=null){
				return true;
			}else{
				return false;
			}
