...
//		System.out.println(getNullColumns(currentStarList));
		...
		if(nullColumns==null){
			...
			int xMove = 0;
			for(int i = nullColumns.get(0);i<MAX_COLUMN_SIZE;i++){
				if(nullColumns.contains(i)){
					...
					for(int j = 9;j>=0;j--){
						...
						if(currentStarList.lookup(j,i)!=null){
							Star s = currentStarList.lookup(j,i);
							MovedStar ms=new MovedStar(new Position(s.getPosition().getRow(),s.getPosition().getColumn()),s.getType(),new Position(s.getPosition().getRow(),s.getPosition().getColumn()-xMove));
							moveStars.add(ms);	
						}
					}
				}
